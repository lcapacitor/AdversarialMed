...
class FGSMAttack(object):
    ...
    def perturb(self, X, y, epsilons=None):
        """
        ...
        X_pt = X + self.epsilon * grad_sign
        X_pt = torch.clamp(X_pt, 0, 1)
...
def to_var(x, requires_grad=False, volatile=False):
    ...
    if torch.cuda.is_available():
        x = x.cuda()
    return Variable(x, requires_grad=requires_grad, volatile=volatile).unsqueeze(0)