...
def train(clean_dir, adv_dir, attack_type, usePixelVAE, limit):
    ...
    if usePixelVAE:
        ...
        pixel_cnn_kernel_size = 3
        pixel_cnn_padding = pixel_cnn_kernel_size // 2
        C, H, W = channel_num, image_size, image_size

        model = models.PixelVAE(C, H, W, z_size, vae_depth, C, OUTCN, pixel_cnn_num_layers, pixel_cnn_kernel_size,
                                pixel_cnn_padding, feature_maps).to(device)
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, targets in tqdm(dataloaders[phase], desc='{} iterations'.format(phase), leave=False):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    if usePixelVAE:
                        ...
                        rec_loss = cross_entropy(reconstructed, (targets * 255).long(), reduce=False).view(b, -1).sum(
                            dim=1)
                ...
                if usePixelVAE:
                    running_kl_loss += torch.sum(kl_loss)
                    running_rec_loss += torch.sum(rec_loss)
                    running_total_loss += torch.sum(total_loss)
                else:
            ...
            if (epoch + 1) % visual_interval == 0 and epoch > 0 and phase == 'val':
                if usePixelVAE:
                    ...
                    sample_zs = model.encoder(inputs[rndIdx:rndIdx+1])
                    sample_zs = vae_util.sample(*sample_zs)

                    sample = vae_util.draw_sample(inputs[rndIdx:rndIdx+1], model.decoder, model.pixcnn, sample_zs, seedsize=(0, 0))
                    generated = sample.data.cpu()

                    adv_input = inputs[rndIdx:rndIdx+1].data.cpu()

                    target = targets[rndIdx:rndIdx+1].data.cpu()


                    utils.save_image(torch.cat((adv_input, generated, target), dim=0), './trained_records/figures/pixelvae_{:02d}.png'.format(epoch), padding=10)
                else:
                    rndIdx = random.randint(0, inputs.size(0) - 1)
    ...
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    weight_fname = 'vae_{}_zdim{}_hdim{}_e{}_lr{}.torch'.format(attack_type, z_size, hidden_dim, num_epochs,
                                                                str(lr).split('.')[-1])
    ...
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    weight_fname = 'vae_{}_zdim{}_hdim{}_e{}_lr{}.pkl'.format(attack_type, z_size, hidden_dim, num_epochs,
                                                              str(lr).split('.')[-1])
    s_path = os.path.join(save_path, weight_fname)
    torch.save(best_model_wts, s_path)
