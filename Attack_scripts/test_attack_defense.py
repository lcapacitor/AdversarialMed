...
PNEU_ADV  = {'fgsm': 'models/pneu_adv_model_fgsm_0.7.ckpt',
             'i-fgsm': 'models/pneu_adv_model_i-fgsm_0.7.ckpt',
             'pgd': 'models/pneu_adv_model_pgd_0.7.ckpt',
             'fgsm-pgd': 'models/pneu_adv_model_fgsm_pgd_0.7.ckpt'}
...
def testPerformance(attack_type, epsilon, data_path, model_type, defense_type, adv_type):
    # Initialize Defense
    bits_squeezing = BitSqueezing(bit_depth=5)
    median_filter = MedianSmoothing2D(kernel_size=3)
    jpeg_filter = JPEGFilter(20)
        ...
        for att in attack_type:
            ...
            if defense_type is not None:
                ...
                for defs in defense_type:
                    if defs.lower() == "jpeg":
                    ...
                    elif defs.lower() == "adv_learn":
                        model_defs = util.loadPneuModel(PNEU_ADV[adv_type])
                        ...
                        defense_input = torch.zeros((inputs.shape)).to(device)
                        ...
                        for j in range(7):
                            for i in range(7):
                                ...
                                _, rec_parts_adv = vaeModel(adv_img[:,:,row_s:row_e,col_s:col_e])
                                _, rec_parts_ori = vaeModel(inputs[:,:,row_s:row_e,col_s:col_e])
                                defense_adv[:,:,row_s:row_e,col_s:col_e] = rec_parts_adv
                                defense_input[:,:,row_s:row_e,col_s:col_e] = rec_parts_ori
                        # Show rec images
    ...
    prc = average_precision_score(gt_labels, pred_probs)
    ...
    print('Clean Examples: Accuracy: {:.4f}, AUC: {:.4f}, PRC: {:.4f}, Recall: {:.4f}, Precision: {:.4f}, f1: {:.4f}'
        .format(accuracy, auc, prc, recall, precision, f1))
    plt.figure(figsize=(10, 8))
    plt.plot(fpr, tpr, '-.', label='clean (auroc={:.4f}, auprc={:.4f}, f1={:.4f})'.format(auc, prc, f1))
    ...
    for att in attack_type:
        ...
        prc_adv = average_precision_score(gt_labels, preds_attacks[att]['pred_probs_adv'])
        ...
        print('{} Adversarial Examples: Accuracy: {:.4f}, AUC: {:.4f}, PRC: {:.4f}, Recall: {:.4f}, Precision: {:.4f}, f1: {:.4f}'
            .format(att, accuracy_adv, auc_adv, prc_adv, recall_adv, precision_adv, f1_adv))
        #plt.plot(fpr_adv, tpr_adv, '-.', label='att: {} (auroc={:.4f}, auprc={:.4f}, f1={:.4f})'
        #    .format(att, auc_adv, prc_adv, f1_adv))

        if defense_type is not None:
            for defs in defense_type:
                auc_defense = roc_auc_score(gt_labels, preds_attacks[att][defs]['pred_probs_defense'])
                prc_defense = average_precision_score(gt_labels, preds_attacks[att][defs]['pred_probs_defense'])
                auc_adv_defense = roc_auc_score(gt_labels, preds_attacks[att][defs]['pred_probs_adv_defense'])
                prc_adv_defense = average_precision_score(gt_labels, preds_attacks[att][defs]['pred_probs_adv_defense'])
                ...
                print('{} defense on Clean Examples: Accuracy: {:.4f}, AUC: {:.4f}, PRC: {:.4f}, Recall: {:.4f}, Precision: {:.4f}, f1: {:.4f}'
                    .format(defs, accuracy_defense, auc_defense, prc_defense, recall_def, precision_def, f1_def))
                print('{} defense on Adversarial Examples: Accuracy: {:.4f}, AUC: {:.4f}, PRC: {:.4f}, Recall: {:.4f}, Precision: {:.4f}, f1: {:.4f}'
                    .format(defs, accuracy_adv_defense, auc_adv_defense, prc_adv_defense, recall_adv_def, precision_adv_def, f1_adv_def))

                plt.plot(fpr_defense, tpr_defense, '-.', label='def: {} on clean (auroc={:.4f}, auprc={:.4f}, f1={:.4f})'
                    .format(defs, auc_defense, prc_defense, f1_def))
                plt.plot(fpr_adv_defense, tpr_adv_defense, '-.',label='def: {} on {} (auroc={:.4f}, auprc={:.4f}, f1={:.4f})'
                    .format(defs, att, auc_adv_defense, prc_adv_defense, f1_adv_def))
...
def main(args):
    ...
    testPerformance(attack_type, epsilon, data_path, model_type, defense_type, adv_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--adv_type', type=str, default='fgsm', help='specify which adv_train to use: fgsm, i-fgsm')
