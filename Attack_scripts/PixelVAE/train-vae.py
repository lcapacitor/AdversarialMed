...
from torchvision.transforms import CenterCrop, ToTensor, Compose, Lambda, Resize, Grayscale, Pad, Resize, Normalize
...
def go(arg):
    ...
    if arg.model == 'vae-up':
        ...
        encoder = models.ImEncoder(in_size=(H, W), zsize=arg.zsize, depth=arg.vae_depth, colors=C)
        decoder = vae_util.Lambda(lambda x : x) # identity
    ...
    testbatch = vae_util.readn(testloader, n=12)
            ...
            with torch.no_grad():
                ...
                if not arg.skip_test:
                    for i, (input, _) in enumerate(tqdm.tqdm(testloader)):
                        ...
                        rec_loss = cross_entropy(rec, target, reduce=False).view(b, -1).sum(dim=1)
                        rec_loss_bits = rec_loss * vae_util.LOG2E  # Convert from nats to bits
