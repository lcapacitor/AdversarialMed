...
class ImEncoder(nn.Module):
    ...
    def __init__(self, in_size, zsize=32, use_res=False, use_bn=False, depth=0, colors=3):
        ...
        # - Encoder
        modules = [
            vae_util.Block(colors, a, use_res=use_res, batch_norm=use_bn),
            nn.MaxPool2d((p, p)),
            vae_util.Block(a, b, use_res=use_res, batch_norm=use_bn),
            nn.MaxPool2d((q, q)),
            vae_util.Block(b, c, use_res=use_res, batch_norm=use_bn),
            nn.MaxPool2d((r, r)),
        ]

        for i in range(depth):
            modules.append( vae_util.Block(c, c, use_res=use_res, batch_norm=use_bn))
...
class ImDecoder(nn.Module):
    ...
    def __init__(self, in_size, zsize=32, use_res=False, use_bn=False, depth=0, out_channels=60):
        ...
        modules = [
            ...
            vae_util.Reshape((c, in_size[0] // (p*q*r), in_size[1] // (p*q*r)))
        ]

        for _ in range(depth):
            modules.append( vae_util.Block(c, c, deconv=True, use_res=use_res, batch_norm=use_bn) )


        modules.extend([
            nn.Upsample(scale_factor=r, mode=upmode),
            vae_util.Block(c, c, deconv=True, use_res=use_res, batch_norm=use_bn),
            nn.Upsample(scale_factor=q, mode=upmode),
            vae_util.Block(c, b, deconv=True, use_res=use_res, batch_norm=use_bn),
            nn.Upsample(scale_factor=p, mode=upmode),
            vae_util.Block(b, a, deconv=True, use_res=use_res, batch_norm=use_bn),
...
class PixelVAE(nn.Module):
    ...
    """
    def __init__(self, c, h, w, zsize, depth, colors, condition_length, pixel_cnn_num_layers, pixel_cnn_kernel_size, pixel_cnn_padding, feature_maps):
        super().__init__()
        in_size = (h, w)
        pixelnn_size = (c ,h ,w)

        self.encoder = ImEncoder(in_size=in_size, zsize=zsize, use_res=True, use_bn=True, depth=depth, colors=colors)
        self.decoder = ImDecoder(in_size=in_size, zsize=zsize, use_res=True, use_bn=True, depth=depth,
                                   out_channels=condition_length)
        self.pixcnn = LGated(pixelnn_size, condition_length, feature_maps, num_layers=pixel_cnn_num_layers, k=pixel_cnn_kernel_size,
                               padding=pixel_cnn_padding)
    ...
    def forward(self, x):
        ...
        out = self.decoder(z)
        rec = self.pixcnn(x, out)

        return (zs[0], zs[1]), rec